        ...
        62353: (t, e, i) => {
            ...
            const v = r().View.extend({
                ...
                showTwitchBroadcasterDialog(t) {
                    ...
                    e += "<div class='success'>Вы успешно подключили свой аккаунт к Jackbox Audience Kit Twitch Extension.</div>", this.lacksAudience ? e += "<div class='warning'>У ЭТОЙ ИГРЫ НЕТ ВХОДА ДЛЯ ЗРИТЕЛЕЙ</div>" : d.app.client.roomInfo.audienceEnabled || (e += "<div class='warning'>У ЭТОЙ КОМНАТЫ ОТКЛЮЧЕНЫ ЗРИТЕЛИ</div>"), h.b.show("custom", {
                        html: e,
                        position: "bottom",
                        timer: t,
                            ...
                            if (s()(".ugc-option").hide(), "RoomFull" !== o)
                                if ("GameLocked" !== o) {
                                    if (u && "Lobby" === u) {
                                        ...
                                        if (!n.isAllowedToStartGame) return e.$el.find("#quiplash-lobby-text").html("Откинься на спинку кресла и отдохни!"), void e.showScreen("#state-lobby");
                                        const a = t.lobbyState;
                                        "WaitingForMore" === a ? e.$el.find("#quiplash-lobby-text").html("Ждём когда все игроки зайдут") : "CanStart" === a ? (e.$el.find("#quiplash-lobby-text").html("Нажми эту кнопку когда все зашли"), e.$el.find("#quiplash-startgame").show()) : "Отсчёт" === a ? (e.$el.find("#quiplash-lobby-text").html("Нажми эту кнопку чтобы отменить запуск игры"), e.$el.find("#quiplash-stopcountdown").show()) : "PostGame" === a && (e.$el.find("#quiplash-lobby-text").html("What do you want to do?"), s()(".quiplash-endbuttons").show()), e.showScreen("#state-lobby")
                                        ...
                                        if ("Gameplay_AnswerQuestion" === o) {
                                            ...
                                            if (e.doneAnswering = !1, e.$el.find("#quiplash-answer-input").val(""), e.$el.find("#quiplash-submit-answer").removeAttr("disabled"), e.$el.find("#quiplash-answer-field").show(), e.$el.find("#quiplash-submit-alert").hide(), e.$el.find("#quiplash-submit-answer").show(), e.$el.find("#quiplash-audience-censor").hide(), !0 === t.safetyQuip ? e.$el.find("#quiplash-submit-safetyquip").show() : e.$el.find("#quiplash-submit-safetyquip").hide(), n.canCensor && e.$el.find("#quiplash-audience-censor").show(), e.$el.find("#state-answer-question #question-text").html(n.question.prompt.replace(/<BLANK>/gi, "________")), e.$el.find("#state-answer-question #question-text-alt").html(""), void 0 !== n.question.quip ? (e.$el.find("#state-answer-question #question-text-alt").html(n.question.quip.replace(/<BLANK>/gi, "________")), e.$el.find("#state-answer-question #question-text").removeClass("quiplash-text"), e.$el.find("#state-answer-question #question-text").addClass("quiplash-text-big")) : (e.$el.find("#state-answer-question #question-text").addClass("quiplash-text"), e.$el.find("#state-answer-question #question-text").removeClass("quiplash-text-big")), n.showError) {
                                                ...
                                                e.$el.find("#quiplash-submit-alert").html("Ты ввёл тоже самое что и другой игроков! Попробуй ещё раз."), t.addClass("alert-info"), t.removeClass("alert-danger"), t.show()
                                            }
                                            e.prevAnswerQuestionId = n.question.id, e.currentAnswerQuestionId = n.question.id, e.showScreen("#state-answer-question")
                                        } else e.$el.find("#quiplash-answer-input").val(""), e.$el.find("#quiplash-submit-answer").removeAttr("disabled"), e.$el.find("#quiplash-answer-field").hide(), e.$el.find("#quiplash-submit-alert").hide(), e.$el.find("#quiplash-submit-safetyquip").hide(), e.$el.find("#quiplash-submit-answer").hide(), e.$el.find("#quiplash-audience-censor").hide(), t.audienceQuip && t.canDoAudiencePlay ? (e.$el.find("#state-answer-question #question-text").html(t.audienceQuip.replace(/<BLANK>/gi, "________")), e.$el.find("#quiplash-answer-field").show(), e.$el.find("#quiplash-submit-answer").show()) : e.$el.find("#state-answer-question #question-text").html("Ты зритель! Жди когда можно будет голосовать."), e.showScreen("#state-answer-question");
                                    else if ("Gameplay_Vote" === u) {
                                        if (e.$el.find("#state-vote #question-text-alt").html(""), !n.canCensor && n.doneVoting) return e.$el.find("#state-vote #vote-text").html("Подожди других игроков!"), e.$el.find("#quiplash-vote").html(""), void e.showScreen("#state-vote");
                                        ...
                                        const i = n.censorOnly || n.doneVoting ? "" : "Какой ответ тебе больше нравится?";
                                        ...
                                        let i = "Какой ответ тебе больше нравится?";
                                        if (n.currentVote) switch (n.currentVote) {
                                            case 0:
                                                i = "Дай золотую медаль своему самому любимому ответу!";
                                                break;
                                            case 1:
                                                i = "А теперь дай серебренную медаль!";
                                                break;
                                            case 2:
                                                i = "И выдай бронзовую медаль своему третьему по любви ответу."
                                        }
                                        e.$el.find("#state-vote #vote-text").html(i);
                                        let o = "",
                                            l = [];
                                ...
                                } else r.b.show(Error("Игра в процессе. Подожди следующую игру."), {
                                    willClose: () => {
                                        window.location.reload(!0)
                                    }
                                });
                        else r.b.show(Error("Комната полная"), {
                            willClose: () => {
                                window.location.reload(!0)
                            }
                        });
...
//# sourceMappingURL=sourcemaps/6190.bb56f1157361e8e9ce60.js.map
